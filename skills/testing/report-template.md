...
> ⚠️ **硬门禁：** 未执行原因**只能**从枚举中选一项，后跟一句 ≤30 字的事实性补充。
> **禁用词：** `已确认` `人工确认` `看起来` `应该` `基本` `大致` `外观` `等价覆盖` `正常` `差不多`
> （命中即视为伪造，报告打回重写）
...
| TC-F001-L2a-03 | {场景} | L2a | 被其他用例阻塞（等 TC-L2b-05 修复） |

**禁止示例：**
- ❌ `工具未配置；L3 截图已人工确认布局正常` — 夹带未发生的"人工确认"
- ❌ `本次跳过；由 L2b 等价覆盖` — "等价覆盖"是结论不是原因；若真等价，在 ✅ 用例证据栏写映射
- ❌ `外观基本正常` — 无证据的推断
