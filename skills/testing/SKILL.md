---
name: codeman-testing
description: "CodeMan 测试验证 Skill。当 CodeMan 工作流进入测试阶段时使用。先根据 PRD 生成 L2-L5 测试用例和自动化脚本，用户确认后逐层执行。由 Orchestrator Skill 调度，或用户说'开始测试'、'测试验证'时触发。"
---

# CodeMan 测试验证 Skill

## 宿主环境说明

**CODEMAN_HOME** 路径规则：Cursor 为 `~/.cursor/skills/.codeman`，Claude Code / OpenCode 为 `~/.claude/skills/.codeman`，Trae 为 `~/.trae/skills/.codeman`。

---

## 概述

本 Skill 分三大阶段，遵循 **ATDD（验收测试驱动开发）** 模式：**编码前生成测试用例 → 编码后编写测试脚本 → 逐层执行**。测试基准来源于 PRD，而非代码。

```
阶段一：用例生成（编码前）     阶段二：脚本编写 + 执行（编码后）
┌─────────────────────────┐    ┌─────────────────────────────────┐
│ 读取 PRD 验收标准          │    │ 读取已锁定的测试用例              │
│ → 生成 L2-L5 测试用例     │    │ → 为每个用例编写自动化脚本         │
│ → 隐性需求推导             │    │ → 代码审查预检 → [门禁]            │
│ → 五项质量校验             │    │ → L1-L5 逐层执行 → [门禁]          │
│ → 验收标准理解确认          │    │ → 防虚假通过 + 证据审计            │
│ → 用户确认用例 → 锁定      │    │ → 生成测试报告                    │
│                           │    │                                   │
│ ⚠️ 此时代码还未编写        │    │ ⚠️ 此时代码已编写完成              │
│ 用例纯粹基于 PRD，不受     │    │ 脚本基于已锁定用例 + 实际代码      │
│ 代码实现影响               │    │ 编写（选择器、API 路径等）          │
└─────────────────────────┘    └─────────────────────────────────┘
```

**关键区别（对比传统流程）：** 测试用例在编码前就锁定，编码的目标是"让代码通过这些用例"，而不是"写完代码再想测什么"。这确保测试基准不被代码实现带偏。

### 分阶段加载（重要）

**本 Skill 的详细规则按阶段拆分到子文件中，避免一次性加载过多规则导致遗忘。进入每个阶段时，必须读取对应的子文件：**

| 阶段 | 触发时机 | 读取文件 | 内容 |
|------|---------|---------|------|
| 阶段一（用例生成） | **设计完成后、编码开始前** | `{CODEMAN_HOME}/skills/testing/phase1-generation.md` | L2-L5 用例生成、隐性需求推导、五项校验、验收标准理解确认 |
| 阶段二（脚本编写 + 执行） | **编码完成后** | `{CODEMAN_HOME}/skills/testing/phase2-execution.md` | 自动化脚本编写、逐层执行、门禁自检、回归策略 |
| 生成测试报告时 | 执行完成后 | `{CODEMAN_HOME}/skills/testing/report-template.md` | 报告模板、填写规范 |

**本主文件中的规则（防虚假通过、描述质量标准、脚本归因等）贯穿所有阶段，不需要额外加载。**

---

## 测试分级

**根据变更规模自动选择测试级别，避免小改动也走完整 L1-L5。** 在测试开始前由 orchestrator 或本 Skill 根据场景判定：

| 级别 | 适用场景 | 执行范围 | 预估 Token |
|------|---------|---------|-----------|
| **轻量** | Bug 修复 / 样式调整 / 单文件改动 | L1 单元测试 + L3 相关用例回归 | ~2K |
| **标准** | 普通迭代 / 新功能（≤3 个功能点） | L1 → L2 → L3，L4/L5 可选（执行前确认） | ~8K |
| **完整** | 大版本 / 上线前 / 用户指定 | L1 → L2 → L3 → L4 → L5 + 兼容性 | ~15K |

**判定规则：**
- orchestrator 场景四（轻量快速通道）→ 轻量测试
- orchestrator 场景一/二/五 → 标准测试（用户可升级为完整）
- 用户主动说"完整测试"/"上线前测试" → 完整测试

**轻量测试简化规则：**
- 阶段一只为受影响的功能点生成/更新用例，不全量生成
- 阶段二只执行 L1（受影响模块的单元测试）+ L3（受影响功能的 E2E 回归）
- 跳过 L2a/L2b/L4/L5/兼容性
- 防虚假通过规则仍然生效

---

## 核心原则：自动化优先 + E2E 优先

**能自动化的一律自动化，有 UI 的功能一律用 E2E 浏览器测试。人工介入是最后手段。**

| 原则 | 说明 |
|------|------|
| E2E 优先 | 有 UI 的功能，优先用 Playwright 在浏览器中端到端测试，不只测 API |
| 测试数据通过浏览器创建 | 需要前置数据时，用 Playwright 模拟用户操作创建（如通过注册页面创建用户），不只靠数据库 seed |
| 测试数据自动生成 | 同时提供 seed/fixture 脚本作为快速初始化手段 |
| UI 操作自动化 | 用 Playwright/Midscene 脚本模拟用户操作，不标记为"待人工" |
| API 调用自动化 | 用测试框架脚本直接调用，不手动 curl |
| 断言自动化 | 每个预期结果都写成代码断言，不用"人工确认是否正确" |
| 降级有明确边界 | 只有当自动化工具本身不可用时才暂停确认，不是"不确定就跳过" |

**E2E 优先的具体规则：**
- PRD 功能点有 UI → 必须有 L3 E2E Playwright 脚本（不只是 L2 API 测试）
- 需要创建测试数据 → 优先通过 Playwright 操作 UI 创建（如填写表单、点击按钮），其次用 seed 脚本
- 数据验证 → 通过 Playwright 在浏览器中验证页面展示的数据，不只验证 API 返回值
- 交互流程（注册→登录→操作→验证）→ 用一个完整的 E2E 脚本串起来

**标记 `[待人工]` 的唯一条件：** 自动化工具无法安装或运行（如 Playwright 无法启动浏览器），且没有其他自动化替代方案。不是因为"不想写脚本"或"不好自动化"。

---

## 核心原则：空缺 > 伪造（最高优先级）

**测试报告可以有空洞、可以残缺、可以很多 ⏳，但不能有任何一句"看起来没问题 / 已确认 / 等价覆盖"式的填空。**

- 触发审查的不是"还有多少 ⏳"，而是"有没有假话"
- 发现某项要填但没做过 → **老实写"未执行"**，不要自行用"听起来合理"的话填满
- 格式完整永远次于事实真实。宁可报告看起来残缺，也不要让它看起来完整但夹带伪造
- 讨好性输出（让报告看起来专业、让各栏都有内容）是虚假通过的主要心理诱因，必须警惕

> **反例（本规则诞生的源头）**：某次迭代 L4 Midscene 未配置，AI 在 ⏳ 行"说明"栏填了"L3 截图已人工确认布局正常"——
> 实际 AI 没有看任何截图。这是为了"让报告完整"而编造细节的典型讨好性输出。

---

## 防虚假通过（不可妥协）

**测试通过的唯一标准是实际执行结果，不是代码审查推断。** 这是本 Skill 最重要的约束。

### 各层级通过的唯一证据

| 测试层级 | 通过的唯一证据 | 严禁行为 |
|---------|-------------|---------|
| L1 单元测试 | 测试框架（pytest/vitest/jest）的实际运行输出 | 仅凭"代码逻辑正确"判定通过 |
| L2 集成测试 | API 脚本或 DB 脚本的实际运行输出 + 响应体/查询结果 | 仅凭代码审查判定接口正确 |
| L3 E2E 测试 | Playwright 脚本实际运行通过 + 截图证据 | 仅凭"页面代码已渲染 xxx"判定通过 |
| L4 UI 视觉 | Midscene.js 断言通过 + Playwright 截图对比结果 | 仅凭"样式看起来正确"判定通过 |
| L5 非功能 | 工具命令实际运行输出（Lighthouse/k6/npm audit） | 仅凭"已配置 xxx"判定通过 |

### 禁止行为清单

以下行为视为**虚假通过**，严格禁止：

1. **推断式通过**：用"代码已实现 xxx""逻辑看起来正确""应该能正常显示"等推断性语言标记通过
2. **跨层级标记**：在代码审查阶段将 L2/L3/L4/L5 用例标记为通过（代码审查只能发现问题，不能证明通过）
3. **无证据通过**：测试报告中「实际结果」栏为空或只有推断描述
4. **环境不可用时标通过**：环境/工具不可用时，必须标记 `⏳ 待验证`，严禁标记 ✅
5. **只验证存在不验证正确**：L3 只检查元素 `toBeVisible()` 不校验布局/交互；L2 只检查 `status 200` 不校验响应数据格式
6. **笼统描述**：测试报告中的「实际结果」「问题描述」「失败原因」使用模糊、概括性语言，未写明具体差异点（详见下方描述质量最低标准）
7. **只跑 API 不验页面**：功能涉及 UI 展示，但仅执行了 API 接口测试或代码审查就判定通过，未通过 Playwright 实际打开页面验证渲染结果

### 描述质量最低标准

**所有测试报告中的描述性字段（实际结果、问题描述、失败原因、失败详情）必须达到以下最低具体度，否则视为不合格报告：**

#### 笼统 vs 具体 对照表（填写前必须自检）

| 类型 | ❌ 笼统描述（禁止） | ✅ 具体描述（要求） |
|------|-------------------|-------------------|
| **UI 布局** | "页面布局有问题""布局不对" | "表单容器宽度 520px 且左对齐，PRD ASCII 原型要求居中且 max-width 400px" |
| **UI 样式** | "按钮样式不对""颜色有误" | "提交按钮圆角为 0px，PRD 要求 8px；背景色为 #ccc，设计稿要求 #1890ff" |
| **文案错误** | "文案不对""文字有误" | "注册按钮文案为 'Submit'，PRD §3.2 要求为 '立即注册'" |
| **API 错误** | "接口返回错误""接口异常" | "POST /api/users 返回 422，body.errors[0].field='email'，缺少 message 字段（API 文档 §2.1 定义必须包含）" |
| **字段缺失** | "返回参数缺少字段" | "GET /api/profile 响应体缺少 avatar 字段，API 文档定义为 string\|null，实际响应中该字段完全不存在" |
| **功能错误** | "功能不正常""计算有误" | "商品数量输入 3、单价 29.9 时，合计显示 89.69，预期为 89.70（精度丢失，PRD §4.1 要求保留两位小数四舍五入）" |
| **交互异常** | "交互有问题""点击没反应" | "点击「提交」按钮后无 loading 状态，页面无任何响应，3 秒后直接跳转到列表页，PRD 要求显示 loading → success toast → 跳转" |
| **数据格式** | "数据格式不对" | "日期字段 created_at 返回 '2024-1-5'，API 文档要求 ISO 8601 格式 'YYYY-MM-DDTHH:mm:ssZ'" |
| **对齐问题** | "没对齐""UI 没对齐" | "表单标签列宽度不一致：'用户名' 标签宽 56px、'电子邮箱' 标签宽 72px，导致输入框左边缘未对齐，PRD 原型要求标签右对齐 + 输入框左边缘统一" |
| **响应式** | "移动端显示有问题" | "375px 视口下侧边栏未折叠，与主内容区重叠，PRD 响应式要求 §5.1 规定 768px 以下侧边栏应自动隐藏为汉堡菜单" |

#### 描述最低要素

每条描述必须包含以下要素，缺一不可：

1. **定位**：哪个页面 / 哪个接口 / 哪个组件 / 哪个字段
2. **实际值**：实际观察到的具体值 / 具体行为 / 具体现象
3. **期望值**：PRD / API 文档 / 设计稿中规定的具体值 / 行为
4. **来源**：期望值的出处（PRD 章节号 / API 文档章节 / 设计稿位置）

**自检公式**：一条合格的描述 = `在 [定位] 中，实际为 [实际值]，但 [来源] 要求为 [期望值]`

> 示例：在注册页表单容器中，实际宽度为 520px 且左对齐，但 PRD §3.1 ASCII 原型要求居中且 max-width 400px。

### 标记 ✅ 前的强制自检

**每个用例标记为 ✅ 通过之前，必须逐项确认以下条件全部为「是」：**

- [ ] 我是否**实际执行**了验证操作（运行脚本 / 发起请求 / 浏览器运行），而非仅审查代码？
- [ ] 「实际结果」栏填写的是**真实观察到的输出**（脚本日志 / API 响应体 / 截图内容），而非推断？
- [ ] 对于 L3/L4 用例，是否有 Playwright 运行结果或截图作为证据？
- [ ] 对于 L2 用例，是否已对照文档校验了响应数据格式（不只是状态码）？
- [ ] 该用例是否在其**对应的执行层级**被标记（而非在代码审查阶段提前标记）？
- [ ] 「实际结果」和「问题描述」是否满足描述质量最低标准（定位 + 实际值 + 期望值 + 来源，四要素缺一不可）？
- [ ] 对于涉及 UI 展示的功能，是否通过 Playwright **实际打开了页面**验证渲染结果，而非仅依赖 API 返回正确就推断页面正常？

**任意一项为「否」→ 该用例必须保持 ⏳ 状态，不得标记 ✅。**

### ⏳ 待验证行的填写约束（硬门禁）

**⏳ 行最容易成为伪造重灾区**——因为"用例没执行"本身不扣分，但 AI 会为了让报告看起来完整，在"说明/原因"栏编造"已人工确认""外观 OK""由其他层等价覆盖"等无证据的话。

**强制格式：** ⏳ 用例**只允许**两个字段，不允许任何附加叙述：

| 字段 | 允许内容（枚举 + 可选补充） |
|------|---------------------------|
| 状态 | `⏳` |
| 未执行原因 | **只能从以下枚举中选一项**：`工具未配置` / `环境不可用` / `超出本次测试范围` / `人工决定跳过` / `被其他用例阻塞`<br>后面可附**一句**不超过 30 字的事实性补充（例如具体工具名、阻塞用例号），**不得**出现"已确认""正常""等价""应该""看起来""外观""大致"等推断性词 |

**禁止行为：**
- ❌ `⏳ | Midscene 未配置；L3 截图已人工确认布局正常` —— 夹带未发生的"人工确认"
- ❌ `⏳ | 本次跳过；由 L2b + L3 等价覆盖` —— "等价覆盖"是结论不是原因；若真等价覆盖，应在 ✅ 用例里写清楚映射
- ❌ `⏳ | 外观基本正常` —— "基本正常"无证据

**正确示例：**
- ✅ `⏳ | 工具未配置（Midscene.js 本迭代未安装）`
- ✅ `⏳ | 超出本次测试范围（L5 压测推迟到上线前）`
- ✅ `⏳ | 被其他用例阻塞（等 TC-L2b-05 修复后验证）`

**若确实存在等价覆盖关系，写法应为：** 在 **✅ 用例的"验证证据"栏**写明"同时覆盖 TC-L2a-01 的场景：表结构由 TC-L2b-02 的 PUT→GET 路径间接验证"，而不是把这句话塞进 ⏳ 行。

### 报告提交前的"伪造扫描"自检（硬门禁）

**报告写完、展示给用户之前，必须逐项自检，任何一项不过 → 报告打回重写：**

- [ ] **扫描全报告**，搜索以下**禁用词**。命中即必须改写或删除：
  - `人工确认` / `已确认` / `看起来` / `应该` / `基本` / `大致` / `外观` / `应该没问题` / `等价覆盖` / `等价` / `正常` / `差不多` / `大概`
  - 例外：用在引用用户原话、描述错误期望值时可保留（例如"PRD 要求正常跳转"）
- [ ] **所有 ⏳ 行的"原因"字段**只含上表枚举值 + 30 字以内事实补充，无任何推断性词汇
- [ ] **所有 ✅ 行的"验证证据"栏非空**，且指向具体命令输出 / 响应体摘要 / 截图路径
- [ ] **✅ 用例与执行日志（见 phase2-execution.md）一一对应**：日志里没有执行记录的用例不得标 ✅
- [ ] **"各层通过分布"中提到的运行命令**，都能在执行日志中找到对应时间戳

**自检不通过时的处理：** 修改后重新自检，不得带着"只是一句话而已"的心态蒙混过关。

### API 通过 ≠ 页面正确：UI 验证强制规则

**当被测功能涉及前端页面展示时，以下行为严格禁止：**

1. **API 正确推断页面正确**：API 返回 200 且数据格式正确 ≠ 页面渲染正确。API 数据可能正确但前端存在：渲染逻辑 bug、CSS 样式错误、组件状态异常、数据绑定错误、条件渲染遗漏
2. **代码审查替代 E2E**：代码中写了 `<div>{data.name}</div>` ≠ 页面上真的显示了 name。必须通过 Playwright 实际运行验证
3. **选择性验证**：只跑了 L2 API 测试就跳过 L3 E2E，声称"后端没问题所以前端应该也没问题"

**强制执行流程：**

```
功能涉及 UI？
├─ 是 → L2 API 测试 ✅ → 仍必须执行 L3 E2E Playwright 测试
│         │                    ├─ 打开实际页面
│         │                    ├─ 验证元素渲染（不只是 toBeVisible，要验内容/布局/交互）
│         │                    └─ 截图存档作为证据
│         └─ L2 API 测试 ❌ → 记录 API 问题 + L3 标记 ⏳ 待 API 修复后验证
└─ 否 → 按正常 L2 流程执行
```

**典型踩坑场景（必须通过 E2E 才能发现）：**

| API 返回正确但页面出错的场景 | 原因 |
|---|---|
| 列表接口返回 10 条数据，页面只显示 5 条 | 前端分页组件 bug，默认 pageSize 错误 |
| API 返回用户名 "张三"，页面显示 "undefined" | 前端取值路径错误，如 `res.data.name` 写成 `res.name` |
| API 返回金额 99.90，页面显示 99.9 | 前端未格式化小数位数 |
| API 返回状态 "active"，页面状态标签未渲染 | 前端状态映射表遗漏该枚举值 |
| API 返回正确的排序，页面顺序错乱 | 前端二次排序逻辑覆盖了后端排序 |
| API 返回图片 URL 正确，页面图片加载失败 | 前端拼接了错误的 CDN 前缀或未处理 null |

---

## 启动判断

执行本 Skill 前，先判断当前是全新测试还是续测。

### 模式 A：全新测试

检查 `.codeman/docs/tests/test-report-latest.md` 是否存在。**若不存在**，进入完整流程（前置条件 → 阶段一 → 阶段二）。

### 模式 B：续测模式（已有测试报告）

**若已存在测试报告**，执行以下步骤：

**① 读取已有报告，统计各状态用例**
- 列出 ✅ 通过、❌ 失败、⏳ 待验证 的数量

**② 对 ✅ 用例做合法性审查（不可跳过）**
- 检查每个标记为 ✅ 的用例，确认其「验证证据」栏有实际执行证据（脚本输出 / API 响应 / 截图）
- 若发现 L2/L3/L4/L5 用例的通过依据是推断性描述（如"代码已实现""逻辑正确"）→ **强制降级为 ⏳**
- 输出降级摘要：降级了多少用例、原因

**③ 检查用例完整性**
- 对照 PRD 功能点和验收标准，检查是否有遗漏的测试场景
- 若有遗漏 → 补充缺失用例后再执行
- 若已完整 → 继续

**④ 增量执行**
- 只执行 ⏳ 和 ❌ 状态的用例，跳过已通过的
- 按 L1 → L2 → L3 → L4 → L5 顺序执行

---

## 前置条件

执行前必须完成：

```
1. 读取 .codeman/docs/prd/INDEX.md
   → 获取所有 PRD 功能点（测试基准来源）

2. 读取相关 PRD 碎片文件 feat-*.md
   → 获取验收标准、边界场景、UI 设计要求

3. 读取 .codeman/docs/tests/INDEX.md（如存在）
   → 了解已有测试覆盖，避免重复

4. 读取 .codeman/config.yaml
   → 了解测试框架配置、技术栈

5. 读取 .codeman/docs/DIRECTIVES.md
   → 加载"需求精粹卡"中的关键验收标准
   → 确保每条关键验收标准都有对应测试用例

6. 读取 .codeman/docs/api/INDEX.md 和 api-*.md
   → 了解 API 接口定义（L2 测试需要）

7. 识别受变更影响但无 PRD 的老模块/老功能（骨架模式或增量迭代时）
   → 检查 prd/INDEX.md，找出本次变更波及但没有 feat-*.md 的现有功能
   → 对这些功能读取代码，提取现有公开行为作为回归测试基线
   → 详见下方「基于代码的回归测试用例生成」章节
```

---

## 基于代码的回归测试用例生成

**触发条件：** 本次变更波及的模块/功能没有对应的 `feat-*.md`（骨架接入后首次迭代，或增量迭代影响了未文档化的老功能）。

**核心原则：** 没有 PRD 就以代码为 source of truth。在**编码前**读取被波及模块的现有代码，基于代码的实际行为生成回归测试用例，锁定的是"改之前的行为"。

### 执行步骤

**Step 1: 定位受影响的老功能**

```
从 design Skill 的影响面分析中，识别出变更波及的模块列表
→ 逐个检查这些模块是否有对应的 feat-*.md
→ 有 PRD：正常从 PRD 生成用例（已有流程）
→ 无 PRD：进入代码回归基线生成
```

**Step 2: 读取代码提取现有行为**

对每个无 PRD 的受影响模块，读取以下内容：

| 读取内容 | 提取什么 | 用于生成什么用例 |
|---------|---------|---------------|
| API 路由定义 | 每个公开接口的方法、路径、参数、响应格式 | L2b API 回归用例 |
| 导出函数/组件 Props | 函数签名、输入输出类型 | L1 单元回归用例 |
| 核心业务逻辑分支 | if/else/switch 中的关键判断条件 | L2b 边界回归用例 |
| 数据校验规则 | validate/check 逻辑中的约束 | L2b 校验回归用例 |
| 前端页面核心元素 | 页面主要展示的数据和交互 | L3 E2E 回归用例 |

**Step 3: 生成回归测试用例**

基于代码行为生成的用例必须遵循以下格式：

```markdown
### TC-REG-{模块}-{序号}: {用例名称}  [regression-from-code]

- **来源**：代码行为回归（非 PRD）
- **被保护的行为**：{改之前的行为描述}
- **代码位置**：{文件:行号}
- **层级**：L{层级}
- **前置条件**：{从代码上下文推断}
- **操作步骤**：{模拟当前行为}
- **预期结果**：{代码当前的实际输出，即改之前的行为}
```

**关键约束：**
- 预期结果 = 代码当前的实际行为（不是"应该怎样"，而是"现在是怎样"）
- 用例标记 `[regression-from-code]`，与基于 PRD 的用例区分
- 编码前生成并锁定，编码后执行时验证老功能是否被破坏
- 不需要覆盖老功能的所有行为，只覆盖**被变更波及**的公开接口和核心逻辑

**Step 4: 向用户确认回归基线**

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
回归测试基线（基于代码）— 请确认
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
以下老功能无 PRD，已从代码提取回归测试基线：

模块：{模块名}（无 feat-*.md）
  TC-REG-{模块}-01: {用例名} — {被保护的行为}
  TC-REG-{模块}-02: {用例名} — {被保护的行为}
  ...共 {N} 个回归用例

这些用例锁定的是当前代码的行为。
编码完成后将执行这些用例，确认老功能未被破坏。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
确认？（或指出遗漏的关键行为 / 调整用例）
```

用户确认后，这些回归用例与 PRD 用例一起锁定为验收基准。

---

## 项目级稳定性验证（三层验证）

**触发条件：** L1-L5 各层级测试全部执行完毕后、Git squash merge 之前。

**核心原则：** 单个功能点的测试通过不等于整个项目稳定。架构变更、模块接口修改可能引发跨模块的连锁问题。三层验证从不同角度确认项目整体可运行。

### V1: 构建验证（零成本，必做）

```bash
# 根据项目技术栈执行对应的构建命令
# TypeScript: tsc --noEmit
# Go: go build ./...
# Python: python -m py_compile {主入口}
# Node.js: npm run build（如有 build script）
# Rust: cargo check
```

**通过标准：** 构建命令退出码为 0，零错误。

**检查范围：** 类型错误、引用断裂、导入缺失、语法错误。这些是架构变更最常见的破坏模式。

### V2: 已有测试套件全量执行（中等成本，必做）

```bash
# 执行项目中所有已有的测试（不只是本次新写的）
# 包括：之前迭代写的测试 + 本次新写的测试 + 代码回归用例
{项目测试命令}  # npm test / go test ./... / pytest
```

**通过标准：** 全部已有测试通过（含之前迭代的测试）。

**意义：** 本次新功能的测试通过只证明新功能正确，已有测试套件全量执行才能发现"新代码是否破坏了老功能"。

### V3: 项目级 Smoke 测试（低成本，必做）

根据项目类型自动选择验证方式：

| 项目类型 | 识别方式 | Smoke 验证内容 |
|---------|---------|--------------|
| Web 后端 | 有 `server/` 或 API 路由 | 启动 dev server → 请求 3-5 个核心接口 → 全部返回 2xx |
| Web 前端 | 有 `src/pages/` 或 SPA 入口 | 启动 dev server → Playwright 打开首页 → 无白屏无控制台错误 |
| 全栈 Web | 前后端都有 | 后端 Smoke + 前端 Smoke 都执行 |
| CLI 工具 | 有 bin/ 或 main 命令 | 执行 help 命令 → 正常输出 → 2-3 个典型参数组合不报错 |
| 库/SDK | 有 index.ts/lib/ 且无 server | import 主入口 → 核心导出 API 可调用 → 无运行时错误 |
| Shell 脚本项目 | 主要是 .sh 文件 | 主脚本 --help 正常输出 + bash -n 语法检查通过 |

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
项目级稳定性验证
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
V1 构建验证：✅ / ❌（{构建命令} → {结果}）
V2 已有测试套件：✅ {N}/{N} 通过 / ❌ {N} 失败（{失败列表}）
V3 项目级 Smoke：✅ / ❌（{验证内容} → {结果}）

整体判定：✅ 三层全过，可合并 / ❌ 有失败项，需修复
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**任一层失败 → 进入修复闭环，修复后重新执行三层验证。三层全过才允许 squash merge。**

---

## 阶段入口

> ⚠️ **测试碎片文件是测试的唯一基准**：阶段二执行时，必须先读取 `.codeman/docs/tests/test-*.md` 碎片文件，
> 按其中定义的用例逐条编写脚本、逐条执行、逐条更新执行状态。不得凭记忆"大概跑一下测试"。
> 测试完成后，必须生成 `test-report-latest.md` 报告文件（见下方报告模板），不得只在对话中口头汇报。

### 进入阶段一：用例生成

⚠️ **必须 Read `{CODEMAN_HOME}/skills/testing/phase1-generation.md`** 并按其中的步骤执行用例生成。
不得跳过此 Read——phase1-generation.md 包含用例生成的完整规则（L2-L5 分层标准、隐性需求推导、断言规范、E2E 选择器优先级等），
这些规则不在本文件中，凭记忆无法覆盖。

### 进入阶段二：执行测试

⚠️ **必须 Read `{CODEMAN_HOME}/skills/testing/phase2-execution.md`** 并按其中的步骤逐层执行测试。
不得跳过此 Read——phase2-execution.md 包含完整的逐层执行流程（代码审查预检 → L1 → L2a → L2b → L3 E2E → L3 网络异常 → L4 UI 视觉 → 兼容性 → L5 非功能 → 项目级稳定性验证 → Git 合并），
以及脚本断言 lint 扫描、脚本冒烟验证等质量保障机制。凭记忆执行会跳过大部分测试层级。

### 生成测试报告

测试完成后，按以下模板生成 `.codeman/docs/tests/test-report-latest.md`（直接使用，不需要 Read 其他文件）：

```markdown
# 测试报告

> 生成时间：{YYYY-MM-DDTHH:MM:SS}
> 版本：{Git commit hash}
> 模式：{全新测试 / 续测（第 N 次）}

## 摘要

| 测试层级 | 用例总数 | ✅ 通过 | ❌ 失败 | ⏳ 待验证 | 通过率 | 自动化率 |
|---------|---------|--------|--------|----------|--------|---------|
| 代码审查 | {N} | {N} | {N} | — | {%} | — |
| L1 单元测试 | {N} | {N} | {N} | {N} | {%} | {%} |
| L2a DB 直查 | {N} | {N} | {N} | {N} | {%} | {%} |
| L2b API 集成 | {N} | {N} | {N} | {N} | {%} | {%} |
| L3 E2E 测试 | {N} | {N} | {N} | {N} | {%} | {%} |
| L4 UI 视觉 | {N} | {N} | {N} | {N} | {%} | {%} |
| L5 非功能 | {N} | {N} | {N} | {N} | {%} | {%} |

**各层通过分布（必须逐层列出，不可笼统合并）：**
- 代码审查发现问题：{N} 个
- L1 单元测试通过：{N} 个（运行命令：`{命令}`）
- L2a DB 直查通过：{N} 个（运行命令：`{命令}`）
- L2b API 集成通过：{N} 个（运行命令：`{命令}`）
- L3 E2E 测试通过：{N} 个（运行命令：`{命令}`，截图证据 {N} 张）
- L4 UI 视觉通过：{N} 个（运行命令：`{命令}`）
- L5 非功能通过：{N} 个（运行命令：`{命令}`）

## 覆盖率

- 单元测试覆盖率：{%}（目标：核心业务 ≥ 80%）
- PRD 验收标准覆盖率：{%}（目标：100%）
- 测试自动化率：{%}（目标：≥ 90%，`[待人工]` 占比 ≤ 10%）

## 用例明细

> ⚠️ **填写规范：**
> - 「实际结果」列必须填写**实际观察到的输出**（脚本日志 / API 响应体 / 截图内容），不得填写"代码已实现""逻辑正确"等推断性文字
> - 「验证证据」列必须填写具体证据（脚本运行输出 / API 响应摘要 / 截图路径），✅ 用例此列不得为空
> - 每个用例必须在其对应层级被标记，不得跨层级标记
> - **⏳ 用例与 ✅/❌ 用例分开列表**，⏳ 行"原因"字段只能从枚举选一项，禁止编造"已人工确认"等推断性说明

### ✅ / ❌ 用例（已执行）

| 编号 | 测试场景 | 层级 | 自动化程度 | 预期结果 | 实际结果 | 验证证据 | 状态 |
|------|---------|------|-----------|---------|---------|---------|------|
| TC-F001-L2b-01 | {场景} | L2b | [全自动] | {预期} | {实际输出} | {脚本输出/响应体} | ✅/❌ |

### ⏳ 用例（未执行，强制枚举格式）

| 编号 | 测试场景 | 层级 | 未执行原因（枚举 + ≤30字事实补充） |
|------|---------|------|--------------------------------|
| TC-F001-L4-01 | {场景} | L4 | 工具未配置（Midscene.js 本迭代未安装） |

## 发现的问题

> ⚠️ **描述质量要求：** 每条问题必须包含「定位 + 实际值 + 期望值 + 来源」四要素，禁止笼统描述。

| 编号 | 定位（页面/接口/组件） | 实际表现 | 期望行为（含来源） | 严重程度 | 发现阶段 | 处理方案 | 状态 |
|------|----------------------|---------|-------------------|---------|---------|---------|------|
| B-01 | {具体页面/接口/组件名} | {实际观察到的具体值或行为} | {PRD §X.X / API 文档 §X / 设计稿要求的具体值或行为} | Critical/Major/Minor | {代码审查/L1/L2/L3/L4/L5} | {修复方案} | ✅ 已修复 / 🔧 待修复 |

## 失败用例

> ⚠️ **失败原因必须具体：** 不得写"测试失败""断言不通过"等笼统原因，必须写明期望值 vs 实际值的具体差异。

| 用例 | 层级 | 脚本路径 | 失败定位 | 期望值 | 实际值 | 来源 | 严重程度 |
|------|------|---------|---------|--------|--------|------|---------|
| {用例名} | L{N} | {脚本路径} | {具体断言/检查点} | {文档规定的值} | {实际观察的值} | {PRD §X / API 文档 §X} | {High/Medium/Low} |

## API 数据格式不符合项

| 接口 | 字段路径 | API 文档定义 | 实际返回 | 严重程度 |
|------|---------|-------------|---------|---------|
| {接口路径} | {如 res.body.profile.avatar} | {文档定义：string\|null} | {实际：number} | {Critical/Major/Minor} |

## UI 布局/交互不符合项

| 页面 | 校验维度 | PRD 原文描述 | 实际表现 | 校验来源 | 严重程度 |
|------|---------|-------------|---------|---------|---------|
| {页面名} | {布局/样式/交互} | {PRD 中的原文描述} | {实际的页面表现} | {ASCII 原型/设计截图/UI 要求} | {Critical/Major/Minor} |

## UI 设计对照

不一致项：{N} 个（布局 {N} + 样式 {N} + 交互 {N} + 数据格式 {N}）
{对照表}

## 网络异常测试结果

> 网络配置：slow_3g（latency={N}ms, download={N}kbps, upload={N}kbps）

| 场景 | 用例数 | ✅ 通过 | ❌ 失败 | 失败详情 |
|------|--------|--------|--------|---------|
| 弱网/慢速 | {N} | {N} | {N} | {用例名}：在 {页面/操作} 中，实际为 {具体现象}，PRD/预期要求 {期望行为} |
| 离线→恢复 | {N} | {N} | {N} | {用例名}：在 {页面/操作} 中，实际为 {具体现象}，PRD/预期要求 {期望行为} |
| 接口超时/错误 | {N} | {N} | {N} | {用例名}：在 {页面/操作} 中，实际为 {具体现象}，PRD/预期要求 {期望行为} |
| 请求中断 | {N} | {N} | {N} | {用例名}：在 {页面/操作} 中，实际为 {具体现象}，PRD/预期要求 {期望行为} |

## 兼容性测试结果

> 测试范围：{用户确认的浏览器和设备列表}

| 浏览器/设备 | L3 E2E | L4 视觉 | 特有问题 |
|------------|--------|---------|---------|
| Chromium 桌面 | ✅ {N}/{N} | ✅ {N}/{N} | — |
| Firefox 桌面 | {状态} | {状态} | {问题描述，如无则 —} |
| WebKit 桌面 | {状态} | {状态} | {问题描述} |
| iPhone 14 | {状态} | {状态} | {问题描述} |
| Pixel 7 | {状态} | {状态} | {问题描述} |
| iPad Pro 11 | {状态} | {状态} | {问题描述} |

## L5 非功能测试详细结果

### 性能测试

**Lighthouse 前端性能（逐页面）：**

| 页面 | LCP | FID | CLS | Score | 状态 |
|------|-----|-----|-----|-------|------|
| {页面路径} | {N}ms（阈值 ≤2500） | {N}ms（≤100） | {N}（≤0.1） | {N}（≥80） | ✅/❌ |

**k6 API 压测：**

| 场景 | 并发 | 持续 | P95 | P99 | 错误率 | 状态 |
|------|------|------|-----|-----|--------|------|
| 基准 | 10 VUs | 30s | {N}ms（≤200） | {N}ms（≤500） | {N}% | ✅/❌ |
| 压力 | 50 VUs | 60s | {N}ms（≤500） | {N}ms（≤1000） | {N}% | ✅/❌ |
| 峰值 | 100 VUs | 10s | {N}ms（≤1000） | — | {N}%（≤1%） | ✅/❌ |

**内存泄漏：** {✅ 无泄漏（增长 {N}MB < 阈值 20MB）/ ❌ 疑似泄漏（增长 {N}MB）/ ⬜ 未执行}

### 安全测试

| 检测项 | 结果 | 详情 |
|--------|------|------|
| 依赖漏洞（npm audit） | ✅/❌ | high: {N}, critical: {N} |
| XSS 检测 | ✅/❌ | {检测数} 个 payload，{通过数} 个拦截 |
| CSRF 检测 | ✅/❌ | {检测数} 个请求，{拒绝数} 个正确拒绝 |
| SQL 注入 | ✅/❌ | {检测数} 个参数，{拦截数} 个正确拦截 |
| 越权访问 | ✅/❌ | 无Token/伪造/过期/越权共 {N} 场景 |
| 敏感信息泄露 | ✅/❌ | {扫描接口数} 个接口，{泄露数} 处泄露 |

**手动测试建议执行情况：** {已执行 N 项 / 全部跳过}

### 可访问性测试

| 页面 | WCAG 级别 | Critical | Serious | Moderate | Minor | 状态 |
|------|----------|----------|---------|----------|-------|------|
| {页面路径} | AA | {N} | {N} | {N} | {N} | ✅/❌ |

## 通过合法性审计（续测模式专用）

> 仅续测模式填写。全新测试可省略此节。

| 用例编号 | 原状态 | 审计结果 | 原因 |
|---------|--------|---------|------|
| {TC-xxx} | ✅ | 维持 ✅ / 降级为 ⏳ | {有脚本输出证据 / 缺乏实际验证证据} |
```

---

## 报告生成后、展示给用户前：伪造扫描自检（强制）

**生成 `test-report-latest.md` 之后、向用户展示之前，必须做一轮机械扫描。任一项不过 → 修改后重新自检。**

### 步骤 1：关键词扫描

对整份报告（尤其是 ⏳ 行"原因"栏、✅ 行"实际结果/验证证据"栏、"发现的问题"的"实际表现/期望行为"栏）搜索以下**禁用词**：

```
已确认 / 人工确认 / 看起来 / 应该 / 基本 / 大致 / 外观
应该没问题 / 等价覆盖 / 等价 / 差不多 / 大概
```

**命中处理：**
- 若该栏描述的是**我实际执行**过的内容 → 改写为具体事实（命令 / 输出 / 截图路径）
- 若该栏描述的是**我没实际执行**过的内容 → 删除，改为 ⏳ + 枚举原因

**保留例外：** 用于引用 PRD/用户原话、描述错误期望值时可保留（如"PRD 要求正常跳转"）。

### 步骤 2：✅ 用例与执行日志对账

- 打开 `.codeman/docs/tests/execution-log.md`
- 对每个 ✅ 用例，在日志中查找"TC-编号 + ✅"的记录
- **找不到 → 该用例强制降级为 ⏳**（原因填"未在执行日志中登记"）

### 步骤 3：⏳ 行格式校验

对每个 ⏳ 用例，检查"原因"字段：
- 是否**只包含**以下枚举之一：`工具未配置` / `环境不可用` / `超出本次测试范围` / `人工决定跳过` / `被其他用例阻塞`
- 后附的补充说明是否 ≤30 字且为**事实性描述**（具体工具名 / 具体阻塞用例号等）
- 不符合 → 改写为合规格式

### 步骤 4：输出自检摘要

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
测试报告伪造扫描自检
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
关键词扫描：命中 {N} 处 → 已处理 {N} 处（{具体处理：改写为事实 / 降级为 ⏳}）
✅ 与日志对账：{N} 个 ✅ 全部匹配 / {N} 个强制降级
⏳ 格式校验：{N} 个 ⏳ 全部合规 / {N} 个已修正
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
自检结果：✅ 通过 / ❌ 未通过（需重写）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**自检未通过 → 必须修改报告后重新从步骤 1 扫描，不得以"只是一句话"为由跳过。**

---

## 不确定即停

遇到以下情况必须暂停，严禁自行推测：

- PRD 验收标准描述模糊，不确定测试用例的预期结果
- 测试用例的前置条件不明确（如不确定测试数据应该是什么状态）
- 不确定某个 API 的正确响应格式（文档与实际不一致）
- 发现 PRD 与技术方案矛盾（如 PRD 说"A"，实现是"B"）
- 测试失败但不确定是 bug 还是测试用例写错了（必须先执行下方「脚本失败归因流程」排查）
- 不确定某个边界场景应该怎么验证
- **PRD 文字描述、ASCII 原型图、设计截图/Figma 链接之间存在冲突**（如原型图显示按钮在左，设计稿显示按钮在右）
- **API 文档与技术文档中的响应格式定义不一致**（如字段类型或结构不同）

**暂停格式：**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ 需人工确认
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
关于测试"{不确定点}"，我理解到以下几种可能：

1. {理解A}：[对应的测试方式/预期结果]
2. {理解B}：[对应的测试方式/预期结果]

请问您的意思是哪种？
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

---

## 脚本失败归因流程

**当测试脚本执行失败时，必须先排查脚本本身是否有 bug，再判定是否为产品 bug。严禁不经调查就将失败结果直接报告为产品问题。**

**为什么：** 测试脚本也是代码，也会有 bug。如果脚本本身有问题（选择器错误、操作路径假设错误、等待时机不对），把脚本 bug 误报为产品 bug 会浪费开发时间，更严重的是会动摇整个测试体系的可信度。

### 强制归因流程（每次脚本失败必须执行）

```
脚本执行失败
│
├─ Step 1: 查看错误信息，初步分类
│   ├─ 选择器找不到元素（TimeoutError / locator not found）→ 进入 Step 2A
│   ├─ 断言不匹配（expect(...).toBe(...) failed）→ 进入 Step 2B
│   └─ 脚本运行报错（语法错误 / 运行时异常）→ 进入 Step 2C
│
├─ Step 2A: 选择器问题排查
│   ├─ 打开页面截图，确认目标元素是否实际存在
│   ├─ 如果元素不存在 → 是否需要前置操作（登录/切换Tab/展开面板）？
│   │   ├─ 是 → 脚本 bug（缺少前置操作步骤），修复脚本后重跑
│   │   └─ 否 → 产品 bug（功能未实现/元素未渲染），记录为产品问题
│   ├─ 如果元素存在但选择器不匹配 → 脚本 bug（选择器写错），修复脚本后重跑
│   └─ 如果元素存在但被遮挡/不可交互 → 检查是否有弹窗/遮罩层 → 可能是产品 bug 或脚本需要先关闭遮罩
│
├─ Step 2B: 断言不匹配排查
│   ├─ 截图确认页面实际显示内容
│   ├─ 对比：脚本期望值 vs 页面实际值 vs PRD 规定值
│   │   ├─ 脚本期望值 ≠ PRD 规定值 → 脚本 bug（期望值写错），修复脚本
│   │   ├─ 页面实际值 ≠ PRD 规定值 → 产品 bug，记录为产品问题
│   │   └─ 页面实际值 = PRD 规定值 但 ≠ 脚本期望值 → 脚本 bug（PRD 理解错误），修复脚本
│   └─ 特别注意：文案断言失败时，先确认实际文案是什么，不要猜测
│
├─ Step 2C: 脚本运行报错排查
│   ├─ 语法错误 → 脚本 bug，修复语法
│   ├─ 导入/依赖问题 → 环境问题，修复配置
│   └─ 运行时异常（如 null 引用）→ 脚本 bug（未正确处理异步/等待），修复脚本
│
└─ Step 3: 记录归因结果
    ├─ 脚本 bug → 修复脚本，重新执行，本次失败不计入测试报告
    └─ 产品 bug → 记录到测试报告的「发现的问题」表，附截图证据
```

### 脚本 bug vs 产品 bug 判定标准

| 判定为脚本 bug | 判定为产品 bug |
|---|---|
| 选择器写错（元素存在但没选中） | 元素确实不存在（功能未实现） |
| 缺少前置操作步骤（没有先切换Tab/登录） | 前置操作正确但元素仍未出现 |
| 期望值与 PRD 不一致（PRD 写"立即注册"，脚本期望"注册"） | 页面实际值与 PRD 不一致 |
| 等待时间不足导致超时（元素延迟加载但最终出现） | 等待足够时间后元素仍未出现 |
| 操作顺序错误（先点了子菜单再点父菜单） | 操作顺序正确但交互行为异常 |
| 测试数据准备不完整（依赖数据未创建） | 数据已正确准备但功能处理异常 |

### 脚本修复后的重新验证

**脚本 bug 修复后，必须重新执行并确认通过，不能直接标记为 ✅：**

1. 修复脚本中的问题
2. 重新运行修复后的脚本
3. 确认通过后，在测试报告中记录：
   - 原始失败原因（脚本 bug：{具体原因}）
   - 修复内容（修改了选择器/增加了等待/补充了前置操作）
   - 修复后结果（重跑通过/发现新的产品 bug）

---

## 脚本冒烟验证（写完脚本后、正式执行前）

**E2E 脚本编写完成后，正式批量执行前，必须先对每个脚本做冒烟验证，确认脚本本身能正常运行：**

### 验证步骤

```typescript
// 每个 E2E 脚本正式执行前，先运行冒烟验证：
// 1. 验证关键选择器能定位到元素
const elementCount = await page.locator('{关键选择器}').count();
console.log(`选择器 '{关键选择器}' 匹配到 ${elementCount} 个元素`);
// 如果 count = 0 → 选择器有问题，先修复再继续

// 2. 验证页面基础状态
await page.goto('{目标URL}');
await page.screenshot({ path: 'tests/screenshots/smoke-{用例名}.png' });
// 截图确认页面正常加载，不是空白页/报错页

// 3. 验证前置操作路径可达
// 如果用例需要先登录、先切换Tab等，验证这些前置操作能成功执行
```

### 冒烟不通过时的处理

| 冒烟结果 | 处理方式 |
|---------|---------|
| 选择器匹配 0 个元素 | 打开页面截图，确认实际 DOM 结构，修正选择器 |
| 页面加载失败/空白 | 检查 URL 是否正确、服务器是否启动、是否需要登录 |
| 前置操作失败 | 检查操作路径是否正确，是否有未预期的弹窗/拦截 |
| 冒烟通过但正式执行失败 | 进入「脚本失败归因流程」排查 |

**冒烟验证通过后，才进入正式的批量测试执行阶段。**

---

## 产出清单

| 产出 | 路径 | 说明 |
|------|------|------|
| 测试用例文档 | `.codeman/docs/tests/test-*.md` | 每个功能点的测试用例（L2-L5） |
| 测试数据脚本 | `tests/fixtures/seed.js` | 测试数据自动初始化 |
| Mock 数据 | `tests/fixtures/mocks.js` | 外部服务 Mock |
| 测试环境脚本 | `tests/setup.sh` | 测试环境自动初始化 |
| L2a DB 直查脚本 | `tests/db/*` | 数据库表结构/迁移/初始数据验证 |
| L2b API 集成脚本 | `tests/integration/*.test.js` | 可执行的 API 测试 + 格式校验 |
| L3 E2E 测试脚本 | `tests/e2e/*.spec.ts` | Playwright 自动化脚本 |
| L3 网络异常脚本 | `tests/e2e/network/*.spec.ts` | 弱网/离线/超时/中断测试 |
| L4 UI 视觉脚本 | `tests/visual/*.spec.ts` | Midscene + Playwright 脚本 |
| 兼容性测试配置 | `tests/playwright.config.ts` | 多浏览器 + 多设备项目配置 |
| L5 非功能脚本 | `tests/nonfunctional/*` | 性能/安全/可访问性脚本 |
| 测试报告 | `.codeman/docs/tests/test-report-latest.md` | 最新测试结果（含验证证据和合法性审计） |
| 覆盖矩阵 | `.codeman/docs/tests/INDEX.md` | 功能点→测试用例映射（L1-L5） |
| STATUS 更新 | `.codeman/docs/STATUS.md` | 测试进度更新 |

---

## 完成后

**首先执行项目级稳定性验证（不可跳过）：**

L1-L5 各层级测试全部通过后，在 Git 合并前执行三层验证（详见上方「项目级稳定性验证」章节）：
- V1: 构建验证 → V2: 已有测试套件全量执行 → V3: 项目级 Smoke 测试
- 三层全过 → 继续 Git 合并
- 任一层失败 → 进入修复闭环 Skill，修复后重新验证

**全部通过（含稳定性验证）：**

首先执行 **Git 合并**（如 config.yaml 的 `git.branch_strategy` 为 true）：

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
测试通过 — Git 合并
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
当前分支：{feature 分支名}
分支上的 commit（{N} 个）：
  1. wip(auth): 实现认证模块
  2. wip(order): 实现订单模块
  3. fix(auth): 修复测试发现的问题

合并方式：squash merge → {主分支}
合并后的 commit message：
  feat: {迭代/功能整体描述}

  - 实现 {功能点1}
  - 实现 {功能点2}
  - 修复 {Bug1}
  
  Refs: F{ID1}, F{ID2}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
确认合并？（或修改 commit message / 选择其他合并方式）
```

用户确认后执行：
```bash
git checkout {主分支}
git merge --squash {feature 分支}
git commit -m "{合并 commit message}"
git branch -d {feature 分支}    # 如 config.yaml 的 git.delete_branch_after_merge 为 true
```

**⚠️ 绝对禁止在 commit message 末尾追加 `Co-Authored-By: Claude ...` 行**（详见 orchestrator commit 规范）。即使 AI 助手默认模板包含此行，也必须移除。

然后输出测试报告：
```
测试验证完成！
阶段一：测试用例 {N} 个 + 自动化脚本全部生成
阶段二：逐层执行（每层均通过门禁自检）
  代码审查：发现 {N} 个问题，已修复
  L1 单元测试：{N} 个，通过率 100%
  L2a DB 直查：{N} 个，通过率 100%
  L2b API 集成：{N} 个，通过率 100%
  L3 E2E 测试：{N} 个，通过率 100%
  L4 UI 视觉：{N} 个，通过率 100%
  L3 网络异常：{N} 个，通过率 {%}
  兼容性测试：{浏览器数} 浏览器 × {设备数} 设备，特有问题 {N} 个
  L5 非功能：{N} 个，通过率 100%
PRD 验收标准覆盖率：100%
测试自动化率：{%}（[待人工] {N} 个，占比 {%}）
虚假通过检查：0 个降级
Git：已 squash merge 到 {主分支}（{N} 个 checkpoint → 1 个 commit）

下一步：部署清单生成
是否立即开始？
```

**测试通过后：同步总文档 + 版本文档**

测试全部通过且 squash merge 完成后，碎片文档已稳定，此时统一同步：

**① 同步 PRD-FULL.md（总需求文档）：**
- 读取本次迭代涉及的所有 `prd/feat-*.md`（从 STATUS.md 的迭代记录识别）
- 新功能点 → 追加到 `PRD-FULL.md` 对应分类下
- 修改的功能点 → 替换 `PRD-FULL.md` 中对应功能点的内容
- 文档开头维护功能点目录（按分类：核心/增强/可选）
- 每个功能点之间用 `---` 分隔
- 如果 `PRD-FULL.md` 不存在，创建并写入文件头：`# 产品需求文档（完整版）\n\n> 本文件由 CodeMan 自动维护，包含所有功能点的累积全貌。\n`

**② 同步 DESIGN-FULL.md（总技术设计文档）：**
- 读取本次迭代涉及的所有 `design/mod-*.md` 和 `api/api-*.md`
- 新模块 → 追加到 `DESIGN-FULL.md` 对应章节下
- 修改的模块 → 替换对应模块内容
- 文档结构固定为四大章节：`## 架构概述`、`## 模块设计`、`## API 设计`、`## 数据库设计`
- 数据库表结构从 mod-*.md 的数据模型部分或代码中的 migration/schema 提取，写入「数据库设计」章节（表名、字段、类型、索引、表关系）
- 如果 `DESIGN-FULL.md` 不存在，创建并写入文件头和四大章节骨架

**③ 版本文档收尾（如 STATUS.md 中存在 `iteration_version`）：**
1. 读取本次迭代的所有 `feat-*.md`，合并写入 `iterations/{iteration_version}/PRD.md`
2. 读取本次迭代的所有 `mod-*.md` + `api-*.md`，合并写入 `iterations/{iteration_version}/DESIGN.md`
3. 将 `tests/test-report-latest.md` 复制到 `iterations/{iteration_version}/TEST-REPORT.md`
4. 完善 `SUMMARY.md`：补充测试结果、影响范围、Git 合并信息
5. 更新 `iterations/INDEX.md`

**④ 输出同步确认：**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
文档同步完成
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
总文档已更新：
  PRD-FULL.md      — 产品需求全貌（{N} 个功能点）
  DESIGN-FULL.md   — 技术设计全貌（{N} 个模块 / {N} 个接口 / {N} 张表）
{如有版本文档}
版本文档已生成：
  iterations/{版本名}/  — PRD + 设计方案 + 测试报告 + 摘要
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
下一步：部署清单生成
是否立即开始？
```

**用户确认后，调度下一阶段：** Read `{CODEMAN_HOME}/skills/deploy/SKILL.md` 并按其指令执行。

**有失败用例：**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
测试发现 {N} 个失败用例
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
失败用例：
  ❌ {用例名}（{层级}）— {失败原因}
  ❌ {用例名}（{层级}）— {失败原因}

失败详情见 .codeman/docs/tests/test-report-latest.md

是否立即进入修复闭环？（直接回复即可，或说"先跳过"/"只修 {用例名}"）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**用户确认修复后，调度修复闭环 Skill：** Read `{CODEMAN_HOME}/skills/fix/SKILL.md` 并按其指令执行。
