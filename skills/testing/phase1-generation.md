...
> 本文件是 `testing/SKILL.md` 的子文件。在**技术方案完成后、编码开始前**由主文件指引加载。
...
**本阶段在编码前执行。此时代码还不存在，测试用例纯粹基于 PRD 验收标准和技术方案推导，不受代码实现影响。** 用例锁定后作为编码的验收基准 — 编码的目标是让代码通过这些用例。

**本阶段只生成测试用例（测什么 + 输入 + 预期结果），不编写自动化脚本。** 脚本在编码完成后的阶段二编写（需要实际代码的选择器、API 路径等信息）。
...
**来源 3：领域规则（`DOMAIN-RULES.md` + `mod-*.md`「领域规则」章节）**
- 每个跨模块不变量（INV-*）→ 生成对应的验证测试用例（如 INV-001 余额等式 → 创建/消费/退款后验证余额一致性）
- 每个实体约束 → 生成边界测试用例（如"VIP 只升不降"→ 尝试降级 VIP 应被拒绝）
- 每个跨功能影响 → 生成联动测试用例（如"VIP 变更影响折扣"→ 升级 VIP 后验证折扣是否更新）
- 标注 `[领域规则]` 来源标识

**来源 4：实际代码合规扫描（Grep 扫描，阶段二执行）**
- 此来源**不用于生成新测试用例**，而是用于**检查代码是否严格按文档实现**
- 扫描代码中所有 API 端点 / 路由定义，与 PRD + 技术方案对比
- 发现文档未提及的接口或逻辑 → **标记为违规项，暂停要求用户确认**：
  - 如果确实需要 → 先补充 PRD/技术方案文档，再补充测试用例
  - 如果不需要 → 删除多余代码
  - **严禁**直接为未经设计的代码补测试来"覆盖"它

**三类来源的优先级：** PRD 验收标准 > 技术方案设计决策 > 领域规则不变量。三类来源的用例为必须生成。代码合规扫描用于审计，不用于生成用例。

### 来源 5：代码回归基线（无 PRD 的受影响老功能）

**触发条件：** 本次变更波及的模块/功能没有对应的 `feat-*.md`（骨架接入后首次迭代，或增量迭代影响了未文档化的老功能）。

**与来源 1-3 的区别：** 来源 1-3 为新功能生成测试用例（基于 PRD/设计/规则）。来源 5 为老功能生成回归用例（基于代码实际行为）。两者并行存在，不互斥。

**具体生成流程（详见 SKILL.md「基于代码的回归测试用例生成」章节）：**

1. 从影响面分析中识别出无 PRD 的受影响模块
2. 读取这些模块的公开接口（API 路由、导出函数、组件）
3. 提取当前行为（正常输入→输出、校验逻辑、核心分支）
4. 生成 `[regression-from-code]` 标记的回归用例
5. 向用户确认后锁定

**回归用例的层级分配：**

| 读取的代码内容 | 生成的用例层级 | 用例类型 |
|--------------|-------------|---------|
| API 路由 + Handler | L2b API 集成 | 每个公开接口至少 1 个正向用例 |
| 导出函数/方法签名 | L1 单元测试 | 核心公开函数的输入→输出 |
| 数据校验逻辑 | L2b API 集成 | 校验规则的正反向用例 |
| 前端页面核心元素 | L3 E2E | 页面主要数据展示和交互 |
| 业务逻辑分支 | L2b 或 L1 | 关键 if/switch 分支 |

**回归用例数量控制：** 不追求全覆盖，只覆盖被变更波及的公开接口和核心逻辑。每个受影响模块一般 3-8 个回归用例。

**与 PRD 用例的合并管理：** 回归用例和 PRD 用例写入同一个 `test-*.md` 文件，但用 `[regression-from-code]` 标记区分。测试执行时一起执行，报告中分开统计。
...
3. 产出合规审计报告：
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
代码合规审计（代码实际路由 vs 文档定义）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
文档中定义且代码已实现：
  ✅ POST /api/auth/register — 有文档 + 有用例
  ✅ POST /api/auth/login — 有文档 + 有用例

文档中定义但代码未实现：
  ❌ PUT /api/users/:id — 技术方案有定义，代码中未找到

⚠️ 代码中存在但文档未定义（违规项，需确认）：
  🚫 DELETE /api/auth/logout — PRD/技术方案均未定义
  🚫 GET /api/users/export — PRD/技术方案均未定义
  → 请确认：保留（需先补文档+用例）还是删除？
...
**有 ❌ 项时：先自动补充缺失用例，再继续。**
**有 🚫 项时（文档未定义的代码）：暂停，要求用户确认。保留 → 先补文档再补用例；删除 → 移除多余代码。严禁直接为未经设计的代码补测试。**
...
确认后写入测试用例文档（不编写自动化脚本，脚本在阶段二编写）。

### Step 7: 写入测试用例文档并锁定

用户确认后：
1. 为每个功能点写入 `.codeman/docs/tests/test-{功能名}.md`（只包含用例定义：用例名、前置条件、测试步骤、预期结果、来源标注）
2. 更新 `.codeman/docs/tests/INDEX.md`
3. **不生成自动化脚本** — 脚本需要实际代码的选择器、API 路径等信息，在编码完成后的阶段二编写
...
测试用例已锁定（编码前验收基准）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
共 {N} 个测试用例（L2: {N} / L3: {N} / L4: {N} / L5: {N}）
PRD 验收标准覆盖率：{%}
用例文档：.codeman/docs/tests/test-*.md

下一步：开发实现（编码目标：让代码通过这些用例）
是否立即开始？
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
