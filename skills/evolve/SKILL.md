...
Rules 同步策略：
- **Cursor**：将 `.codeman/rules/{name}.mdc` 复制到 `.cursor/rules/codeman-{name}.mdc`
- **Claude Code**：将 `.codeman/rules/{name}.mdc` 转换为 `.claude/rules/codeman-{name}.md`（frontmatter 中 `globs` 改为 `paths`，移除 `alwaysApply`）

Skills 注册策略：
- **Cursor**：需要用户在 Cursor Settings → Agent → Skills 中手动添加路径
- **Claude Code**：Skills 放在 `.codeman/skills/` 目录下即可自动发现，无需手动注册
...
4. 根据当前宿主环境同步规范：
   ...
   **Cursor 环境**（存在 `.cursor/` 目录）：
   ```bash
   cp .codeman/rules/{name}.mdc .cursor/rules/codeman-{name}.mdc
   ```

   **Claude Code 环境**（存在 `.claude/` 目录）：
   将 `.codeman/rules/{name}.mdc` 转换为 `.claude/rules/codeman-{name}.md`：
   - 保留 `description` 字段
   - 将 `globs` 字段改为 `paths`（Claude Code 的路径匹配字段）
   - 移除 `alwaysApply` 字段
...
4. 根据当前宿主环境提示注册：
   ...
   **Claude Code 环境**：
   Skills 已放在 `.codeman/skills/` 目录下，Claude Code 会自动发现，无需手动注册。
   可直接使用 `/{skill-name}` 斜杠命令调用。
...
存放路径：{路径}
同步状态：
  ✅ 已同步到 .cursor/rules/（Cursor，如适用）
  ✅ 已同步到 .claude/rules/（Claude Code，如适用）
...
| 产出 | 路径 | 说明 |
|------|------|------|
| 新 Rule 文件 | `.codeman/rules/{name}.mdc` | 项目规范（源文件） |
...
| Cursor 同步 | `.cursor/rules/codeman-*.mdc` | Rules 同步（Cursor 环境） |
| Claude Code 同步 | `.claude/rules/codeman-*.md` | Rules 同步（Claude Code 环境） |
