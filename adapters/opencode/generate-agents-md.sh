#!/usr/bin/env bash
# 生成/更新项目级 AGENTS.md 中的 CodeMan 片段（OpenCode 适配）
# AGENTS.md 是 OpenCode 的主规则文件，等同于 Claude Code 的 CLAUDE.md
# 用法：bash generate-agents-md.sh [PROJECT_DIR] [CODEMAN_DIR]
...
'"${PROJECT_NAME}"' 已接入 CodeMan 全流程开发工作流框架。
...
if [ -f "$AGENTS_MD" ]; then
    if grep -q "<!-- CODEMAN START -->" "$AGENTS_MD" 2>/dev/null; then
        # 替换已有片段
        python3 - "$AGENTS_MD" "$CODEMAN_BLOCK" << 'PYEOF'
import sys, re
...
PYEOF
        ...
        echo -e "${GREEN}  已追加 CodeMan 片段到 ${AGENTS_MD}${NC}"
    fi
else
    # 创建新文件
    echo "$CODEMAN_BLOCK" > "$AGENTS_MD"
    echo -e "${GREEN}  已创建 ${AGENTS_MD}${NC}"
fi

echo -e "${YELLOW}  OpenCode 将在每次会话开始时自动加载此文件${NC}"
